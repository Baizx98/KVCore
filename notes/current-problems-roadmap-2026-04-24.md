...
- `ModelRunner` 已经能初始化共享 KV tensor、构建 `PagedAttentionMetadata`、执行模型 forward、取 sample position 并采样。
...
- 新主路径只通过 `attn_metadata` 传共享 KV tensor 和 paged metadata。
...
- `EngineConfig` 已支持 `profile_kv_cache=True` 或 `num_gpu_blocks=None`。
