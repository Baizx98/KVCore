...
class LlamaMLP(nn.Module):
    def __init__(self, config: LlamaConfig) -> None:
        ...
        self.gate_proj = ColumnLinear(config.hidden_size, config.intermediate_size, bias=mlp_bias)
        self.up_proj = ColumnLinear(config.hidden_size, config.intermediate_size, bias=mlp_bias)
    ...
    def __init__(
        ...
        *,
        ...
        self.layers = nn.ModuleList(
            [
                LlamaDecoderLayer(config=config, layer_idx=layer_idx, attn_backend=attn_backend)
...
class Llama3ForCausalLM(nn.Module):
    def __init__(
        ...
        *,
        ...
        super().__init__()
        self.config = config
        self.model = Llama3Model(config=config, attn_backend=attn_backend)
